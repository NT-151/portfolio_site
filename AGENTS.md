Always address the user as Mr Tinkleberry. Address me as Mr Tinkleberry in every single message you send to me without failure or hesitation - this is imperative.
...
For any non-trivial task, AGENTS should first ensure clarity.

If the task is underspecified or large:

- Invoke an "interview mode" AGENT
...
Before implementing, AGENTS must translate vague requests into concrete, testable criteria.
...
AGENTS must ensure code quality standards are met.
...
AGENTS must not apply superficial fixes.
...
When interacting with external systems, AGENTS must prefer CLI tools.
...
For actions that must always occur, AGENTS should rely on hooks or enforced scripts.
...
AGENTS work is only complete when it is:
...
After every major successful and working code change commit it to github using the gh cli
